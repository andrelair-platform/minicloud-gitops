...
### The regulatory layer — compliance by design, not a silo (insurance FDE standard)

ktayl is a **regulated insurer** (Solvency II spine + transversal EU/FR frameworks: DORA, GDPR, EU AI
Act, IDD/DDA, AML/sanctions, IFRS 17, SFDR…). *(Insurer ≠ bank — CRR/CRD/PSD2 do NOT apply.)* So every
capability's discovery **must not stop at business value** — it extends through the regulatory tail.
This is the **compliance-by-design** discipline and the differentiator for an insurance FDE (a generic
AI engineer stops at "value"; the insurance FDE produces the controls + audit evidence).

**The extended FDE discovery chain (mandatory for FDE playbooks + Path-C PRD/architecture):**
```
business process → pain → systems + data → AI/automation → business value
   → REGULATORY IMPACT → CONTROLS REQUIRED → AUDIT EVIDENCE → MONITORING
```

**Rules:**
1. **Regulation is not a silo — one capability triggers many frameworks at once.** An AI Underwriting
   Assistant simultaneously hits AI Act + GDPR + DORA + Solvency II + IDD + Sanctions/AML + Outsourcing.
   The design artefact lists **all** triggered frameworks and the **combined** control set, not one.
2. **Controls are declared at design time, not bolted on at audit.** A Path-C PRD/architecture (and
   every FDE domain-playbook capability) names its triggered frameworks + the controls-by-design; the
   **security/architecture governance gate blocks** work that hasn't.
3. **AI-Act risk tiering is mandatory for any AI use case.** Classify the tier — the controls scale with
   it, and they **spike the moment AI influences a decision with consequences for a person** (a RAG
   knowledge assistant ≠ claims summarisation ≠ fraud signals ≠ UW recommendation ≠ *automated
   pricing/eligibility decision*). Reuse the existing `ai-governance` matrix + Langfuse logging.
4. **Evidence accrues to the control library** (internal-control loop
   `Risk→Control→Owner→Evidence→Testing→Finding→Remediation→Audit`), owned by **Regulatory & Compliance
   (#15)**. The master **obligations register** (frameworks × owner/systems/data/controls/ACPR-EIOPA
   ref/**applicability caveat**) is its map. Note two caveats: **applicability by activity** (IARD vs vie
   vs réassurance differ) and **group-IFRS vs local-statutory**.
