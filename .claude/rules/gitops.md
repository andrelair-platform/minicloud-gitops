...
**Migrating an existing kustomize service — the gotchas (from the ktayl-policy pilot, HGP-07/10):**
- **Immutable Deployment selector — two migration modes:** the chart defaults to an `app.kubernetes.io/*` selector, but the old Deployment's `app:` selector is immutable. **(a) dev / low-stakes → delete-first:** `kubectl delete deploy <name>` once, ArgoCD recreates with the standard selector (brief downtime). **(b) prod / zero-downtime → `selectorLabels` override (chart ≥0.1.4):** set values `selectorLabels: {app: <name>}` to MATCH the live selector → ArgoCD does an **in-place rolling update, no delete, no downtime**. Strictly better than `Replace=true` (which delete+recreates = same downtime). The override label also lands on the pod, so on that env use `selectorLabels` **instead of** `podLabels` (both `{app: <name>}` = duplicate key) and point any `podAntiAffinity`/ServiceMonitor selector at `app: <name>` too. Keep the kustomize overlay as rollback = repoint the Application source.
- **Label-matched NetworkPolicies**: ns netpols select `app: <name>` → dev sets `podLabels: {app: <name>}`; a prod `selectorLabels` flip already puts that label on the pod (else egress denied → app exits at startup).
- **Stateful satellites survive the flip:** a dedicated DB (StatefulSet + PVC) stays a **satellites** source, copied **verbatim** from the retired overlay so its immutable selector + `volumeClaimTemplates` are identical → in-place update, **data preserved, not pruned**. Reference: ktayl-policy prod Postgres (HGP-10).
